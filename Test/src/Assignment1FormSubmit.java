...
public class Assignment1FormSubmit {
	...
	public static void main(String[] args) {
		...
		{
			drv.findElement(By.name("email")).sendKeys("abc");
			//validation if invalid entry is done
			String expectedattribute = drv.findElement(By.xpath("//i[@data-bv-icon-for='email']")).getAttribute("class");
			System.out.println(expectedattribute);
			String actualattribute = "form-control-feedback bv-icon-input-group glyphicon glyphicon-remove";
		...
		if (drv.findElement(By.name("comment")).isDisplayed())
			drv.findElement(By.name("comment")).sendKeys("Test is running");
